...
const Tabs = ({
  ...
  return (
    <nav
      ref={navRef}
      className="flex flex-shrink-0 justify-center items-center relative z-0 py-2 w-full "
    ...
    >
        ...
        return (
          <motion.button
            ...
            className={classNames(
              'text-md relative font-montserrat rounded-md flex items-center h-10 px-4 z-20 bg-transparent cursor-pointer select-none transition-colors',
              {
                'text-white/60': !isActive,
                'text-[#FFCD4D]': isActive,
              },
            )}
            ref={(el) => {
      ...
      {selectedRect && navRect && (
        <motion.div
          className={'absolute z-10 bottom-0 left-0.5 h-[3px] bg-[#FFCD4D] dark:bg-zinc-200 rounded'}
          initial={false}
          animate={{
